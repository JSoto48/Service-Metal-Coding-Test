...
I noticed the website has a broader category to organize the subcategories. Instead of a complex hierarchical category model, I used two simple fields for flexibility:
...
Rather than creating specific columns for each product attribute (size, PSI, material), I used a separate `ProductSpecification` table with `spec_name` and `spec_value` fields.
